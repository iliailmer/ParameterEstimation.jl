...
for datasize in 3:21
    ...
    #                                    solver = solver)
    ParameterEstimation.write_sample(data_sample;
                                     filename = "point_error_data/samples/lv/lv-$datasize.txt")
    # for each in res
    #     estimates = vcat(collect(values(each.states)), collect(values(each.parameters)))
...
# png("point_error_data/figures/lv_t_$(time_interval[1])_$(time_interval[2]).png")
# open("point_error_data/data/lv_t_$(time_interval[1])_$(time_interval[2]).txt", "w") do f
