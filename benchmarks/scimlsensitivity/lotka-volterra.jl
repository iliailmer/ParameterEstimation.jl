...
solution_true = ModelingToolkit.solve(prob_true, solver, p = p_true,
...
prob = ODEProblem(model, ic, time_interval,
                  p_rand)
sol = solve(prob, solver, p = p_rand, saveat = sampling_times)

function loss(p)
    sol = solve(prob, Tsit5(), p = p, saveat = sampling_times)
