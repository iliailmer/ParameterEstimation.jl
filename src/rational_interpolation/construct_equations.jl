...
function construct_poly_equations(identifiability_result::Dict, interpolated_function, diff_order::Int)
    ...
    Et_hc = [HomotopyContinuation.subs(ParameterEstimation.nemo2hc(each), pairs(y_eq)...) for each in Et]
    return Et_hc
end
