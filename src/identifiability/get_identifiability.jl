function get_identifiability(ode::ModelingToolkit.ODESystem; params_to_assess=[], measured_quantities=Array{ModelingToolkit.Equation}[], infolevel=0)
    ...
    if length(params_to_assess) == 0
        ...
        params_to_assess_ = [SIAN.eval_at_nemo(each, Dict(input_syms .=> gens_)) for each in params_to_assess]
        nemo2mtk = Dict(params_to_assess_ .=> params_to_assess)
    end

    res = ParameterEstimation.identifiability_ode(ode_prep, params_to_assess_; p=0.99, p_mod=0, infolevel=infolevel, weighted_ordering=true, local_only=false)
        ...
        if weighted_ordering
            for i in 1:length(Et_hat)
                for _var in Set(vars(Et_hat[i]))
                    ...
                    Et_hat[i] = SIAN.make_substitution(Et_hat[i], _var, _var^get(weights, _var_non_jet, 1), parent(_var)(1))
                end
            end
            for _var in Set(vars(Q_hat))
                _var_non_jet, _var_order = SIAN.get_order_var(_var, non_jet_ring)
                Q_hat = SIAN.make_substitution(Q_hat, _var, _var^get(weights, _var_non_jet, 1), parent(_var)(1))
            end
        end
        # 4. Determine.
        @info "GB computation"
