@testset "Convert Nemo polynomial systems into HomotopyContinuation type" begin
    ...
    E = HomotopyContinutation.System([ParameterEstimation.nemo2hc(e) for e in Et])
    E_native = HomotopyContinutation.System([x + y + z - 1, x + y - 2, x * z - 1])
    ...
    E = HomotopyContinutation.System([ParameterEstimation.nemo2hc(e) for e in Et])
    E_native = HomotopyContinutation.System([x^2 + 1 // 3 * y + z * x - 1,
                                                -x + y^3 * x^2 - 2,
                                                x * z - 1])
    @info "Source System: $Et"
    @info "Converted System: $E"
    ...
    E = HomotopyContinutation.System([ParameterEstimation.nemo2hc(e) for e in Et])
    E_native = HomotopyContinutation.System([
                                                x * y * z * x // 10 - 1,
                                                -x - y - 2,
                                                z * x - 1,
                                            ])
